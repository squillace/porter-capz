...
func TestSchema(t *testing.T) {
	schemaBackup := "../pkg/capz/schema/schema.json.bak"
	schemaPath := "../pkg/capz/schema/schema.json"
	...
	output := &bytes.Buffer{}
	cmd := exec.Command("../bin/mixins/capz/capz", "schema")
	cmd.Stdout = output
	cmd.Stderr = output

	err = cmd.Start()
	require.NoError(t, err, "failed to start the capz schema command")

	err = cmd.Wait()
	t.Log(output)
	require.NoError(t, err, "capz schema failed")
}
